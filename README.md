...
Le fichier reel `.env` ne doit pas etre versionne car il peut contenir des secrets.
...
Le mode hors Docker est optionnel et n'est pas le chemin recommande pour une demonstration. Il necessite PostgreSQL avec pgvector, Redis, Go, Node.js, npm et Goose installes localement.

Pour le backend, les variables utilisees dans `.env.docker.example` peuvent servir de reference, mais les URLs doivent viser les services locaux, par exemple PostgreSQL sur `localhost:5432` et Redis sur `localhost:6379`.
...
Le dossier `doc/` contient uniquement les documents finaux utiles au projet:

| Fichier | Contenu |
|---|---|
| `doc/NutriMatch.pdf` | Description fonctionnelle du projet |
| `doc/dfd.pdf` | Diagramme de flux de donnees |
| `doc/STRIDE.pdf` | Analyse de menaces STRIDE |
| `doc/Rapport_Final.pdf` | Rapport final d'architecture, fonctionnement et securite |
